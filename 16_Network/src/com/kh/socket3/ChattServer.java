...
class ClientThread extends Thread{
	...
	static List<PrintWriter> list = Collections.synchronizedList(new ArrayList<PrintWriter>()) ;	// 다른 사람들까지 다 받아서 넘겨야하기 때문에 + 동기화 필요
	
	public ClientThread(Socket socket) {	//소켓을 받아와야함
		this.socket = socket;
		try {
	...
	public void run() {
		...
		while(br!=null) {
				...
				System.out.println("[" + socket.getInetAddress() + "]가 보낸 메세지");
