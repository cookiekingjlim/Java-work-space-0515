...
public class MemberController {
	...
	private HashMap<String, Member/*여기에 Object가 제일 많이 와*/> map = new HashMap<>();
	...
	public String login(String id, String password) {
		 ...
		 * 
		 * */
		if(map.containsKey(id) && map.get(id)/*키 - id*/.getPassword()/*id에서 가져온 password가*/.equals(password)) {	// 전달 받은 id 존재하는지 & 저장된 비밀번호와 사용자가 입력한 비밀번호 일치하는지 확인
	...
	public boolean changePassword(String id, String oldPw, String newPw) {
		...
		if(map.containsKey(id) && map.get(id).getPassword().equals(oldPw)) {
			map.put(id/*키*/,new Member(newPw, map.get(id).getName())/*두번째는 멤버 객체*/);	//set으로 받아도 됨
			return true;
		}
