...
//		b.autoCasting();
	 ...
	 * - 데이터 범위가 작은 자료형을 큰 자료형으로 변환할 때 (자동으로)사용한다.
	 * */
	public void autoCasting() { // 자동 형 변환 메소드
		byte b = 10; // 1byte
		short s = b; // 2byte
		int i = s; // 4byte
		long l = i; // 8byte
	...
	/*
	 ...
	 - 범위가 큰 크기의 자로형의 데이터를 작은 크기의 자료형으로 변환하려고 할 때 사용한다.
	 - 강제 형 변환의 경우 데이터 손실이 발생할 수 있다.
	*/
	public void casting() {
		double d = 4.12345678904567890; // 15자리수만 표현 가능
		float f = (float) d; // 6자리수까지 표현 가능
		...
		int sum = (int) (i + d);
		int sum2 = i + (int)d; // d를 먼저 정수로 변경 후 계산함
		...
		// char <-> int :  각 문자들마다 고유한 숫자가 지정되어 있기 때문에 쌍방향으로 형 변환 가능하다.(유니코드)
		// char(2byte)는 음수값 저장 불가 -> 값의 범위가 0~65535
		int num = 'A';
